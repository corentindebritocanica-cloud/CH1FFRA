...
- Fonctionnel pour l'instant :
  ...
  - **Écran de chargement au lancement** : le logo se trace comme une découpe laser/plasma (contour lettre par lettre, via `stroke-dashoffset` calculé sur la longueur réelle de chaque tracé), puis se remplit en plein — clin d'œil au métier plutôt qu'un simple spinner. Repli immédiat (logo plein, pas d'animation) si `prefers-reduced-motion` ou si le navigateur ne supporte pas `getTotalLength()`
