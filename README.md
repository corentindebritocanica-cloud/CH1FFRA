...
- Fonctionnel pour l'instant :
  - Formulaire de devis (matière, épaisseur, poids, perte matière %, quantité) avec **plusieurs opérations chaînées** (usinage + tôlerie + soudure sur une même pièce), un **temps de réglage séparé** du temps machine (coût fixe par série, pas par pièce), une **liste de sous-traitance externe** (anodisation, peinture...) et des **frais fixes** (transport/emballage) activables en un clic
  - **Page dédiée "Matières"** : bibliothèque modifiable (ajout, édition, suppression de matières et de catégories entières), avec désignations normalisées (aciers, aciers inox ex. X2CrNiMo17-12-2 / 316L, fontes, aluminiums, plastiques techniques PTFE/POM/PE/PA6-66/PEEK/PMMA/PC/PVC), densité, prix indicatif au kg **et fournisseur habituel + référence** éditables pour chaque matière — toute modification se répercute immédiatement dans le formulaire de devis
  ...
  - Calcul du prix estimé (opérations + matière avec perte + sous-traitance, réglage amorti sur la série, marge, puis frais fixes hors marge)
  - **Comparer avec d'autres matières** : recalcule le même devis avec d'autres matières de la bibliothèque, affichées côte à côte
  - **Comparer par palier de quantité** : affiche le prix unitaire et le prix total pour plusieurs quantités choisies (le coût de réglage s'amortit sur la série)
  ...
  - **Page "Clients"** : liste modifiable (nom, **type** particulier/pro/gros compte, email, téléphone, adresse, notes). Un client saisi dans un devis et qui n'existe pas encore est **créé automatiquement** (fiche minimale, à compléter plus tard) — pas de double saisie. Le champ "Client" du formulaire de devis propose ces clients en autocomplétion (`<datalist>`), avec un lien direct vers la page de gestion
  - **Marge par défaut selon le type de client** (particulier/pro/gros compte), réglable dans Paramètres > Préférences, appliquée automatiquement à la sélection du client (reste ajustable au cas par cas)
  - **Référence pièce** sur chaque devis (autocomplétion depuis l'historique) — permet de retrouver le réel d'une pièce déjà chiffrée
  - **Historique** : affichage de la référence pièce, recherche/filtre par client ou référence, suppression possible d'un devis (bouton ✕ par ligne, confirmation demandée)
  - **Export PDF réel** (jsPDF) : logo, informations client/pièce/référence, détail des opérations/sous-traitance/frais fixes, prix total, mentions légales — testé et fonctionnel
...
- Pas encore fait :
  ...
  - Tableau de bord (CA, taux de conversion), duplication d'un devis existant, export CSV clients
  - Vue "Paramètres" : taux horaire par défaut par opération (pour l'instant seules les marges et frais fixes ont une valeur par défaut)
